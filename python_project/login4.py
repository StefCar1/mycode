    ...
    while usernamecount < 3:
        ...
        while passwordcount < 3:
            ...
            if password == password2:
                ...
                backtomenu = input("Would you like to return to the main menu? ")
                if backtomenu == "yes":
                    main()
                else:
                    exit()
            ...
            for line in account_lines:
                if len(line) > 1 and " | " in line[1]:
                    ...
                    if email == email_info:
                        ...
                        retrieval_menu_answer = input("Would you like to return to the main menu? (yes/no) ").strip().lower()
                        if retrieval_menu_answer == "yes":
                            main()
                        else:
                            print("Goodbye!")  # Fix the typo in Print
                            exit()  # Properly exit the program
                        return  # Added return to exit the function after handling the response

        email_retrieval_count += 1  # Increment after checking all accounts
        if email_retrieval_count == max_attempts:
            bitchindragon()
            print("You've reached the maximum attempts to provide a valid email address.")
            retrieval_account_creation = input("Would you like to create an account? (yes/no) ").strip().lower()
